...
(() => {
  ...
  const LS_KEY = 'leon_image_slots';

  // Public API used by admin.html CMS to set/clear images programmatically.
  window.__imageSlots = {
    set(id, dataUrl) {
      setSlot(id, { u: dataUrl, s: 1, x: 0, y: 0 });
    },
    clear(id) { setSlot(id, null); },
    getAll() { return Object.assign({}, slots); },
  };
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j) => {
        ...
        tombstones.clear();
        // localStorage overrides the state file (set by CMS or a prior drag-drop session)
        try {
          const lsRaw = localStorage.getItem(LS_KEY);
          if (lsRaw) {
            const lsData = JSON.parse(lsRaw);
            for (const id of tombstones) delete lsData[id];
            Object.assign(slots, lsData);
          }
        } catch (e) {}
      })
      .catch(() => {
        // fetch failed (file:// or missing file) — fall back to localStorage only
        try {
          const lsRaw = localStorage.getItem(LS_KEY);
          if (lsRaw) slots = Object.assign({}, JSON.parse(lsRaw), slots);
        } catch (e) {}
      })
  ...
  function save() {
    ...
    // Always persist to localStorage so images survive page reloads locally
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      const gen = ++this._gen;
      try {
        // Use the larger of width/height so portrait/tall slots (e.g. 9:16)
        // aren't downscaled using only the narrow width as the cap target.
        const w = Math.max(
          this.clientWidth || this.offsetWidth || 0,
          this.clientHeight || this.offsetHeight || 0
        ) || MAX_DIM;
